...
class BitSense : public LayerCounter<no_layer, T> {
private:
  ...
  /**
   * @brief Number of bits in upper(>=1) layers
   * 
   */
  size_t rsz;
  /**
...
public:
  ...
  void initBs(const std::vector<size_t> &no_cnt,
                   const std::vector<size_t> &width_cnt,
                   const std::vector<size_t> &no_hash,
                   bool use_negative_counters = false,
                   bool use_cm_sketch = false, size_t cm_row = -1,
  ...
  if (overflow) {
    ...
    if (use_cm_sketch && layer == 0) {
      for (size_t i = 0; i < cm_row; ++i) {
        ...
        cm_sketch[i][ind].getVal() + overflow;
      }
    }
...
BitSense<no_layer, T, hash_t>::BitSense(
    const std::vector<size_t> &no_cnt_, const std::vector<size_t> &width_cnt_,
    const std::vector<size_t> &no_hash_, bool use_negative_counters_,
    bool use_cm_sketch_, size_t cm_row_, size_t cm_width_){
      initBs(no_cnt_, width_cnt_, no_hash_, use_negative_counters_, use_cm_sketch_, cm_row_, cm_width_);
}

template <int32_t no_layer, typename T, typename hash_t>
void BitSense<no_layer, T, hash_t>::initBs(const std::vector<size_t> &no_cnt_,
                   const std::vector<size_t> &width_cnt_,
                   const std::vector<size_t> &no_hash_,
                   bool use_negative_counters_,
                   bool use_cm_sketch_, size_t cm_row_,size_t cm_width_){
  no_cnt = no_cnt_;
  width_cnt = width_cnt_;
  no_hash = no_hash_;
  use_negative_counters = use_negative_counters_;
  use_cm_sketch = use_cm_sketch_;
...
template <int32_t no_layer, typename T, typename hash_t>
T BitSense<no_layer, T, hash_t>::getCntVal(size_t index) const {
  if (index >= no_cnt[0]) {
    throw std::out_of_range("Index Out of Range: Should be in [0, " +
                            std::to_string(no_cnt[0] - 1) + "], but got " +
                            std::to_string(index) + " instead.");
...
template <int32_t no_layer, typename T, typename hash_t>
size_t BitSense<no_layer, T, hash_t>::getOfNum(int32_t lr) const{
...
void BitSense<no_layer, T, hash_t>::print_rate(const char *name) {
  ...
  for (int32_t i = 0; i < length; i++) {
    if (getStatus(i) == true) {
      overflow_num++;
      are += 1.0 * std::abs(getCntVal(i) - getOriginalCnt(i)) /
             ((getOriginalCnt(i) >> width_cnt[0]) << width_cnt[0]);
    }
    if (getCntVal(i) == getOriginalCnt(i)) {
      correct_num++;
    }
  }
  if (use_cm_sketch) {
