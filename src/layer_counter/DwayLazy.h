...
/**
 * @brief Counter layers used in DwayLazy
 * 
...
public:
   ...
   */
  DwayLazyCntLayer(const std::vector<size_t> &no_cnt, const std::vector<size_t> &width_cnt);
  /**
   * @brief Destructor
   *
   */
 ...
 */
template <int32_t no_layer, typename T>
class DwayLazy : public LayerCounter<no_layer, T> {
#ifdef TEST_DWAYLAZY
public:
#else
private:
   ...
   * 
   */
  // std::unique_ptr<DwayLazyCntLayer<no_layer, T>> cnt_ptr;
  /**
   * @brief Number of counters on each layer, from low to high.
   *
   */
  ...
  DwayLazy(const DwayLazy &) = delete;
  DwayLazy(DwayLazy &&) = delete;
  /**
...
public:
  ...
  /**
   * @brief Construct dway_lazy sharing structure and initialize inner counters.
   * 
   * @param counter_num Number of counters you wish to use.
   * @param dway_lazy Number shared segments in each layer.
   * @param width_cnt Counter width of each layer.
   * 
   */
  DwayLazy(size_t counter_num, size_t group_num, const std::vector<size_t> &dway_lazy,
        const std::vector<size_t> &width_cnt){
    initCounter(counter_num, group_num, dway_lazy, width_cnt);
  }
  ...
  /**
   * @brief Release DwayLazy Counters
   * 
   */
  ~DwayLazy(){}
  ...
  void initCounter(size_t counter_num, size_t group_num,
      const std::vector<size_t> &dway_lazy, const std::vector<size_t> &width_cnt);
...
template <int32_t no_layer, typename T>
DwayLazyCntLayer<no_layer, T>::DwayLazyCntLayer(
      const std::vector<size_t> &no_cnt,
      const std::vector<size_t> &width_cnt)
...
template <int32_t no_layer, typename T>
size_t DwayLazyCntLayer<no_layer, T>::getUnusedNum(int32_t layer) const{
  if(layer==0){return 0;}
  size_t result = 0;
...
template <int32_t no_layer, typename T>
size_t DwayLazy<no_layer, T>::getUnusedNum(int32_t layer) const{
  if(layer==0){return 0;}
  size_t result = 0;
  for(auto idx:tag_array[layer]){
    if(idx==DTAG_INVALID){result++;}
...
template <int32_t no_layer, typename T>
void DwayLazyCntLayer<no_layer, T>::clearAll(){
  for (int32_t lr = 0;lr<no_layer;++lr){
    for (auto& seg:cnt_array[lr]){
      seg.reset();
    }
...
template <int32_t no_layer, typename T>
size_t DwayLazyCntLayer<no_layer, T>::bits_num(size_t tag_len) const{
...
template <int32_t no_layer, typename T>
size_t DwayLazyCntLayer<no_layer, T>::tag_bits(size_t tag_len) const{
  size_t bits = 0;
  for(int32_t lr = 1; lr<no_layer; ++lr){
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::initCounter( size_t counter_num,
    size_t group_num, const std::vector<size_t> &dway_lazy,
  ...
  if (di.size() != no_layer) {
    throw std::invalid_argument(
        "Invalid Argument: `dway_lazy` should be of size " +
        std::to_string(no_layer) + ", but got size " +
  ...
  for (size_t i = 1;i<di.size();++i) {
    if (di[i] == 0) {
      throw std::invalid_argument(
          "Invalid Argument: There is a zero in `dway_lazy`.");
    }
  }
  for (auto i : width_cnt) {
    if (i == 0) {
  ...
  // cnt_ptr = std::make_unique<DwayLazyCntLayer<no_layer, T>>(no_cnt, width_cnt);
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::update(size_t ori_index, T val){
  update_num+=1;
    ...
    if(of_val!=0){
      if(lr==no_layer-1){ // last layer should not overflow
        throw std::overflow_error(
            "Counter overflow at the last layer in dway_lazy counter, overflow by " +
...
template<int32_t no_layer, typename T>
inline void DwayLazy<no_layer, T>::insert_backup(int32_t layer, size_t index, T of_val)
{
  T val = of_val;
...
template <int32_t no_layer, typename T>
std::pair<T, int32_t> DwayLazy<no_layer, T>::query_with_layer(size_t ori_index){
  size_t index = (ori_index*pseed)%cNum;
...
template <int32_t no_layer, typename T>
T DwayLazy<no_layer, T>::query(size_t ori_index){
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::clear_cnt(size_t ori_index){
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::decode(){
...
template <int32_t no_layer, typename T>
size_t DwayLazy<no_layer, T>::bsize() const{
  size_t tag_len = ceil(log2(gNum));
  size_t bits = no_cnt[0]*width_cnt[0];
  for(int32_t lr = 1; lr<no_layer; ++lr){
    bits+=no_cnt[lr]*(width_cnt[lr]+tag_len);
...
template <int32_t no_layer, typename T>
size_t DwayLazy<no_layer, T>::tagsize() const{
  size_t tag_len = ceil(log2(gNum));
  size_t bits = 0;
  for(int32_t lr = 1; lr<no_layer; ++lr){
    bits+=no_cnt[lr]*tag_len;
...
template <int32_t no_layer, typename T>
size_t DwayLazy<no_layer, T>::csize(const std::vector<size_t>& idxs) const{
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::dumpOfIdx(std::ostream& os) const{
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::dumpFreeCnt(std::ostream& os) const{
  for(int32_t lr = 0;lr<no_layer;++lr){
...
template <int32_t no_layer, typename T>
void DwayLazy<no_layer, T>::dumpCntSize(std::ostream& os) const{
  size_t unused_bits = 0;
  for(int32_t lr=1;lr<no_layer;++lr){
    unused_bits += getUnusedNum(lr)*(width_cnt[lr]);
  }
