...
private:
  ...
  Counter::DwayNeg<LAYERNUM, COUNTER_TYPR> counter;
  int32_t counter_num;
  std::vector<Ptr> testPtr;

public:
  TestDwayNeg(const std::string_view config_file_): config_file(config_file_){
    counter_num = 0;
  }
...
void TestDwayNeg::initPtr(toml::array& sketch_list, 
