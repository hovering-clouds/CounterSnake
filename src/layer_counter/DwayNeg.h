...
/**
 * @brief Counter layers used in DwayNeg
 * 
...
public:
   ...
   */
  DwayNegCntLayer(const std::vector<size_t> &no_cnt, const std::vector<size_t> &width_cnt);
  /**
   * @brief Destructor
   *
   */
 ...
 */
template <int32_t no_layer, typename T>
class DwayNeg : public LayerCounter<no_layer, T> {
#ifdef TEST_DWAYNEG
public:
#else
private:
   ...
   * 
   */
  std::unique_ptr<DwayNegCntLayer<no_layer, T>> cnt_ptr;
  /**
   ...
   */
  void reset_inner_cnt(size_t index);
...
public:
   ...
   * 
   */
  DwayNeg(size_t counter_num, size_t group_num, const std::vector<size_t> &dway,
        const std::vector<size_t> &width_cnt){
  ...
  /**
   * @brief Release DwayNeg Counters
   * 
   */
  ~DwayNeg(){}
...
template <int32_t no_layer, typename T>
DwayNegCntLayer<no_layer, T>::DwayNegCntLayer(
      const std::vector<size_t> &no_cnt,
      const std::vector<size_t> &width_cnt)
...
template <int32_t no_layer, typename T>
size_t DwayNegCntLayer<no_layer, T>::getUnusedNum(int32_t layer) const{
  if(layer==0){return 0;}
  size_t result = 0;
...
template <int32_t no_layer, typename T>
void DwayNegCntLayer<no_layer, T>::clearAll(){
...
template <int32_t no_layer, typename T>
size_t DwayNegCntLayer<no_layer, T>::bits_num(size_t tag_len) const{
...
template <int32_t no_layer, typename T>
size_t DwayNegCntLayer<no_layer, T>::tag_bits(size_t tag_len) const{
  size_t bits = 0;
  for(int32_t lr = 1; lr<no_layer; ++lr){
...
template <int32_t no_layer, typename T>
void DwayNeg<no_layer, T>::initCounter( size_t counter_num,
  ...
  cnt_ptr = std::make_unique<DwayNegCntLayer<no_layer, T>>(no_cnt, width_cnt);
  sign_bits = std::vector<bool>(cNum, false);
...
template <int32_t no_layer, typename T>
void DwayNeg<no_layer, T>::reset_inner_cnt(size_t index){
...
template <int32_t no_layer, typename T>
void DwayNeg<no_layer, T>::decode(){
...
template <int32_t no_layer, typename T>
size_t DwayNeg<no_layer, T>::bsize() const{
  size_t tag_len = ceil(log2(gNum))+1;
  return cnt_ptr->bits_num(tag_len)/8;
}

template <int32_t no_layer, typename T>
size_t DwayNeg<no_layer, T>::tagsize() const{
  size_t tag_len = ceil(log2(gNum))+1;
  return cnt_ptr->tag_bits(tag_len)/8;
}

template <int32_t no_layer, typename T>
size_t DwayNeg<no_layer, T>::csize(const std::vector<size_t>& idxs) const{
...
template <int32_t no_layer, typename T>
void DwayNeg<no_layer, T>::dumpFreeCnt(std::ostream& os) const{
  for(int32_t lr = 0;lr<no_layer;++lr){
