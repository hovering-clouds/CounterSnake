...
#include "LcTest.h"
#include <sketch/LcDeltoid.h>

#define LC_DT_TEST_PATH "DT.test"
#define LC_DT_PARA_PATH "DT.para"
#define LC_DT_DATA_PATH "DT.data"

namespace OmniSketch::Test {

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t = Hash::AwareHash>
class LcDeltoidTest : public LcTestBase<key_len, no_layer, T> {
...
public:
  ...
  LcDeltoidTest(const std::string_view show_name, const std::string_view config_file,
                Data::StreamData<key_len>& data_, Data::CntMethod method)
      : LcTestBase<key_len, no_layer, T>(show_name, config_file, LC_DT_TEST_PATH, data_, method) {}
...
void LcDeltoidTest<key_len, no_layer, T, hash_t>::initPtr(int32_t counter_num, Counter::LayerCounter<no_layer, T>& counter, Util::ConfigParser& parser){

  /// step i. List Sketch Config
  int32_t num_hash, num_group;
  std::string method;
  parser.setWorkingNode(LC_DT_PARA_PATH);
  ...
  parser.setWorkingNode(LC_DT_DATA_PATH);
  ...
  /// the methods in it
  this->ptr = std::make_unique<Sketch::LcDeltoid<key_len, no_layer, T, hash_t>>(num_hash, num_group, counter_num, counter);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void LcDeltoidTest<key_len, no_layer, T, hash_t>::runTest() {
...
#undef LC_DT_TEST_PATH
#undef LC_DT_PARA_PATH
#undef LC_DT_DATA_PATH