...
#include "BrickTest.h"
#include <sketch/BrickSketchLearn.h>

#define BRICK_SL_TEST_PATH "Brick.SL.test"
#define BRICK_SL_PARA_PATH "Brick.SL.para"
#define BRICK_SL_DATA_PATH "Brick.SL.data"

namespace OmniSketch::Test {

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t = Hash::AwareHash>
class BrickSLTest : public BrickTestBase<key_len, no_layer, T> {
...
public:
  ...
  BrickSLTest(const std::string_view config_file, Data::StreamData<key_len>& data_, Data::CntMethod method)
      : BrickTestBase<key_len, no_layer, T>("Brick SketchLearn", config_file, BRICK_SL_TEST_PATH, data_, method) {}
...
void BrickSLTest<key_len, no_layer, T, hash_t>::initPtr(int32_t counter_num, Counter::Brick<no_layer, T>& counter, Util::ConfigParser& parser){

  /// step i. List Sketch Config
  int32_t depth, width;
  std::string method;
  parser.setWorkingNode(BRICK_SL_PARA_PATH);
  ...
  parser.setWorkingNode(BRICK_SL_DATA_PATH);
  ...
  /// the methods in it
  this->ptr = std::make_unique<Sketch::BrickSketchLearn<key_len, no_layer, T, hash_t>>(depth, width, counter_num, counter);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void BrickSLTest<key_len, no_layer, T, hash_t>::runTest() {
...
#undef BRICK_SL_TEST_PATH
#undef BRICK_SL_PARA_PATH
#undef BRICK_SL_DATA_PATH