...
#include "LcTest.h"
#include <sketch/LcMVSketch.h>

#define LC_MV_PARA_PATH "MV.para"
#define LC_MV_TEST_PATH "MV.test"
#define LC_MV_DATA_PATH "MV.data"
...
class LcMVTest : public LcTestBase<key_len, no_layer, T> {
...
public:
  ...
  LcMVTest(const std::string_view show_name, const std::string_view config_file,
            Data::StreamData<key_len>& data_, Data::CntMethod method)
      : LcTestBase<key_len, no_layer, T>(show_name, config_file, LC_MV_TEST_PATH, data_, method) {}
...
void LcMVTest<key_len, no_layer, T, hash_t>::initPtr(int32_t counter_num, Counter::LayerCounter<no_layer, T>& counter, Util::ConfigParser& parser){
  ...
  /// Step ii. Parse
  parser.setWorkingNode(LC_MV_PARA_PATH);
  if (!parser.parseConfig(depth, "depth"))
    return;
  if (!parser.parseConfig(width, "width"))
    return;

  parser.setWorkingNode(LC_MV_DATA_PATH);
  ...
  /// the methods in it
  this->ptr = std::make_unique<Sketch::LcMVSketch<key_len, no_layer, T, hash_t>>(depth, width, counter_num, counter);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void LcMVTest<key_len, no_layer, T, hash_t>::runTest() {
...
#undef LC_MV_PARA_PATH
#undef LC_MV_TEST_PATH
#undef LC_MV_DATA_PATH
