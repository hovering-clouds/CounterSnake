...
#include "BrickTest.h"
#include <sketch/BrickMVSketch.h>

#define BRICK_MV_PARA_PATH "Brick.MV.para"
#define BRICK_MV_TEST_PATH "Brick.MV.test"
#define BRICK_MV_DATA_PATH "Brick.MV.data"
...
class BrickMVTest : public BrickTestBase<key_len, no_layer, T> {
...
public:
  ...
  BrickMVTest(const std::string_view config_file, Data::StreamData<key_len>& data_, Data::CntMethod method)
      : BrickTestBase<key_len, no_layer, T>("Brick MVSketch", config_file, BRICK_MV_TEST_PATH, data_, method) {}
...
void BrickMVTest<key_len, no_layer, T, hash_t>::initPtr(int32_t counter_num, Counter::Brick<no_layer, T>& counter, Util::ConfigParser& parser){
  ...
  /// Step ii. Parse
  parser.setWorkingNode(BRICK_MV_PARA_PATH);
  if (!parser.parseConfig(depth, "depth"))
    return;
  if (!parser.parseConfig(width, "width"))
    return;

  parser.setWorkingNode(BRICK_MV_DATA_PATH);
  ...
  /// the methods in it
  this->ptr = std::make_unique<Sketch::BrickMVSketch<key_len, no_layer, T, hash_t>>(depth, width, counter_num, counter);
}

template <int32_t key_len, int32_t no_layer, typename T, typename hash_t>
void BrickMVTest<key_len, no_layer, T, hash_t>::runTest() {
...
#undef BRICK_MV_PARA_PATH
#undef BRICK_MV_TEST_PATH
#undef BRICK_MV_DATA_PATH
