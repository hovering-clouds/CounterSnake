...
T Deltoid<key_len, T, hash_t>::query(const FlowKey<key_len> &flowkey) const {
  ...
  if (cnt_distrib) {
    ...
    for (int32_t k = 0; k < 32; ++k) {
      std::cout << distrib[k] / num_hash_ / num_group_ / nbits_ << " ";
      if (distrib[k] == 0.0) {
        std::cout << std::endl;
        break;
      }
    ...
    double len = 0.0;
    for (int32_t k = 0; k < 32; ++k) {
      len += (k + 1) * (distrib[k] - distrib[k + 1]) / num_hash_ / num_group_ /
             nbits_;
      if (distrib[k + 1] == 0.0) {
  ...
  for (int32_t i = 0; i < num_hash_; ++i) {
    ...
    for (int32_t j = 0; j < nbits_; ++j) {
      if (flowkey.getBit(j)) {
        ...
        min_val = std::min(min_val, arr1_[i][idx][nbits_]-arr1_[i][idx][j]);
      }
    }
  }
  return min_val;
  ...
  for (int32_t i = 0; i < num_hash_; i++) {
    for (int32_t j = 0; j < num_group_; j++) {
      ...
      for (int32_t k = 0; k < nbits_; k++) {
        ...
        bool t0 = (arr1_[i][j][nbits_]-arr1_[i][j][k] > thresh);
        if (t1 == t0) {
          reject = true;
          break;
        }
...
size_t Deltoid<key_len, T, hash_t>::size() const {
  return sizeof(Deltoid<key_len, T, hash_t>) +
         (num_group_ * num_hash_ * (nbits_ + 1)) * sizeof(T) +
